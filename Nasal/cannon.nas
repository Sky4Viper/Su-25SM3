...
#ammo counter
props.globals.initNode("/controls/armament/roundsLeft", 0.0, "INT");
props.globals.initNode("/controls/armament/roundsCount", 0.0, "DOUBLE");
var reload = func {
	if( getprop("/gear/gear[0]/wow") and getprop("/gear/gear[1]/wow") and getprop("/gear/gear[2]/wow") and (getprop("/velocities/groundspeed-kt") < 2) ) {
		...
		screen.log.write("GSh-30-2 cannon reloaded (250 rounds)", 1, 0.6, 0.1);
	}
	else {
...
var outOfAmmo = maketimer(1.0, 
	func { 
		...
		setprop("/controls/armament/trigger-GSh-30-2", 0);
...
var triggerControl = func {
	...
	if(triggerState and getprop("/controls/armament/roundsLeft") > 0) {
		...
		if(getprop("/controls/armament/report-ammo"))
			screen.log.write("GSh-30-2 rounds left: " ~ getprop("/controls/armament/roundsLeft"), 1, 0.6, 0.1);
	}
}
